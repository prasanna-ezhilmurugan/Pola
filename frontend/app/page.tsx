...
import { QueryEntity, PolicyResult, ClauseMapping, DocumentHighlight } from '@/lib/types';
...
export default function Home() {
  ...
  const [selectedClause, setSelectedClause] = useState<ClauseMapping | null>(null);
      ...
      >
        <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          <section className="mb-8" aria-labelledby="upload-heading">
            <h2 id="upload-heading" className="text-xl font-semibold text-policy-text mb-4">
              Document Upload
            </h2>
          ...
          <section className="mb-8" aria-labelledby="query-heading">
            <h2 id="query-heading" className="text-xl font-semibold text-policy-text mb-4">
              Policy Query
            </h2>
          ...
          {showResults && result && (
            <section className="mb-8" aria-labelledby="results-heading">
              <h2 id="results-heading" className="text-xl font-semibold text-policy-text mb-4">
                Analysis Results
              </h2>
              <ResultsPanel 
                result={result}
          ...
          {showResults && (
            <section aria-labelledby="viewer-heading">
              <h2 id="viewer-heading" className="text-xl font-semibold text-policy-text mb-4">
                Document Analysis
              </h2>
              <DocumentViewer 
          ...
          {/* Loading State */}
          {isLoading && (
            <section className="mb-8" aria-live="polite" aria-label="Loading results">
              <div className="bg-policy-card rounded-xl p-8 border border-policy-secondary">
                ...
                <div className="mt-6 space-y-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-sm text-policy-text-muted">Processing entities...</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-policy-accent-teal rounded-full animate-pulse" style={{animationDelay: '0.5s'}} />
                    <span className="text-sm text-policy-text-muted">Searching policy documents...</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <div className="w-2 h-2 bg-yellow-500 rounded-full animate-pulse" style={{animationDelay: '1s'}} />
                    <span className="text-sm text-policy-text-muted">Generating recommendations...</span>
                  </div>
                </div>
              </div>
            </section>
